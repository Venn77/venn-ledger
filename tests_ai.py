...
if __name__ == "__main__":
    ...
    def validate_and_save_batch(results, default_currency, categories, payment_methods, vendors):
        for item in results:
            ...
            dt = datetime.datetime(2025, month_res, day_res)

            # 6. Confirm and Save
            confirm = input(f"   >> Save {item['vendor']} ({item['amount']}) to DB? (y/n/skip): ").lower()
            if confirm == 'y':
                try:
                    manager.add_expense(
                        amount=item['amount'],
    ...
    try:
        ...
        for idx in range(0, len(daily_chunks), batch_size):
            ...
            if parsed_results:
                ...
                validate_and_save_batch(parsed_results, currency_str, cat_names, pm_names, ven_names)
